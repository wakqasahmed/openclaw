...
describe("buildAgentSystemPrompt", () => {
  ...
  // The system prompt now includes the actual date/time, which can cause cache instability
  // but is needed to prevent the model from guessing wrong dates.
  // See: https://github.com/openclaw/openclaw/issues/28194
  it("includes the date/time when userTime is provided", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/clawd",
      userTimezone: "America/Chicago",
      userTime: "Monday, January 5th, 2026 — 3:26 PM",
    ...
    // The prompt should contain the formatted date/time string when provided.
